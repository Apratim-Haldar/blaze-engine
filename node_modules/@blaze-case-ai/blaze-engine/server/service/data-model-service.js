...
async function updateDataModel(dataModelId, updateData) {
  ...
  try {
    ...
    // Create a new updatedModel object starting with the existing model
    const updatedModel = {
      ...model,
      id: dataModelId // Ensure ID remains unchanged
    };
    
    // Update basic properties if provided (but don't override with undefined values)
    if (updateData.label) updatedModel.label = updateData.label;
    if (updateData.description !== undefined) updatedModel.description = updateData.description;
    if (updateData.extends) updatedModel.extends = updateData.extends;
    if (updateData.isEditable !== undefined) updatedModel.isEditable = updateData.isEditable;
    
    // Handle attributes intelligently
    if (updateData.attributes && Array.isArray(updateData.attributes)) {
      // Initialize attributes array if it doesn't exist
      if (!updatedModel.attributes) {
        updatedModel.attributes = [];
      }
      
      // Process each attribute in the update data
      updateData.attributes.forEach(newAttr => {
        if (!newAttr.id) return; // Skip attributes without ID
        
        // Check if this attribute already exists
        const existingAttrIndex = updatedModel.attributes.findIndex(attr => attr.id === newAttr.id);
        
        if (existingAttrIndex !== -1) {
          // Update existing attribute by merging
          updatedModel.attributes[existingAttrIndex] = {
            ...updatedModel.attributes[existingAttrIndex],
            ...newAttr
          };
        } else {
          // Add new attribute
          updatedModel.attributes.push(newAttr);
        }
      });
    }
    
    // Handle hooks if present
    if (updateData.hooks && typeof updateData.hooks === 'object') {
      updatedModel.hooks = {
        ...(updatedModel.hooks || {}),
        ...updateData.hooks
      };
  ...
  ${updatedModel.isEditable !== undefined ? `isEditable: ${updatedModel.isEditable},` : ''}
